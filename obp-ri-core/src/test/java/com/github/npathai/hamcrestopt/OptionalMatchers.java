// No copyright, taken from https://github.com/npathai/hamcrest-optional
...
public class OptionalMatchers
{
  ...
  private static class PresenceMatcher extends TypeSafeMatcher<Optional<?>>
  {
    ...
    @Override protected boolean matchesSafely(Optional<?> item)
    {
      return item.isPresent();
    }

    @Override protected void describeMismatchSafely(Optional<?> item,
  ...
  private static class HasValue<T> extends TypeSafeMatcher<Optional<T>>
  {
    ...
    @Override protected boolean matchesSafely(Optional<T> item)
    {
      return item.isPresent() && matcher.matches(item.get());
    }

    @Override protected void describeMismatchSafely(Optional<T> item,
      Description mismatchDescription)
    {
      if(item.isPresent())
      {
